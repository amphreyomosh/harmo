...
const socialLinks = [
  ...
  },
  {
    ...
    href: "https://www.facebook.com/river.happy",
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="currentColor">
  ...
  },
  {
    ...
    href: "https://www.tiktok.com/@amharmo",
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="currentColor">
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {activeProject && (
        ...
        >
          <div
            className="bg-[rgb(18,18,18)] rounded-xl max-w-3xl w-full overflow-hidden shadow-2xl animate-modal-in flex flex-col md:flex-row"
            style={{ maxHeight: "90vh" }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Left — image, full height on desktop */}
            <div className="relative h-56 md:h-auto md:w-[42%] md:flex-shrink-0 overflow-hidden">
              <img
            ...
            {/* Right — content */}
            <div className="flex-1 p-7 md:p-8 overflow-y-auto relative">
              {/* Close button — top right of modal */}
              <button
                onClick={() => setActiveProject(null)}
                className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center text-white bg-white/10 hover:bg-white/20 transition-colors rounded-sm text-xl font-light leading-none"
              >
                &times;
              </button>
              ...
              <div className="flex items-center gap-3 mb-5 pr-10">
                <span className="bg-white text-black text-[10px] font-extrabold px-3 py-1 uppercase tracking-widest">
                  {activeProject.tech[0]}
                </span>
                <span className="text-gray-500 text-sm">{activeProject.label}</span>
              </div>

              {/* Title */}
              <h3 className="font-fraunces text-2xl md:text-3xl font-bold text-white mb-6 leading-snug">
                {activeProject.title}
              </h3>
              ...
              <div className="flex gap-2 flex-wrap mb-8">
                {activeProject.tech.map((t) => (
                  <span key={t} className="text-xs border border-white/15 text-gray-400 px-2.5 py-1 rounded-full">
                    {t}
                  </span>
                ))}
              </div>
              ...
              <div className="flex gap-3 flex-wrap">
                {activeProject.link ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 bg-white text-black font-bold px-6 py-3 rounded-lg text-sm hover:bg-gray-200 transition-all duration-200"
                  >
                  ...
                  <span className="text-sm text-gray-500 font-medium self-center">Coming Soon</span>
                )}
                <button
                  onClick={() => setActiveProject(null)}
                  className="inline-flex items-center gap-2 bg-white/10 border border-white/10 text-white font-semibold px-6 py-3 rounded-lg text-sm hover:bg-white/20 transition-all duration-200"
                >
                  Close
                </button>
              </div>
