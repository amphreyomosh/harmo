...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      {/* Navbar */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-600 p-4 flex justify-between items-center">
        <img
        ...
        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="focus:outline-none">
          <svg className="w-9 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      <div
        className={`fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center transition-transform duration-700 ease-in-out transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <ul className="text-white text-5xl space-y-8 text-center">
