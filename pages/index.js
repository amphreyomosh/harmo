...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 bg-gray-600 p-4 flex justify-between items-center transition-opacity duration-700 ${isMenuOpen ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
        <img
      ...
      <div
        className={`fixed inset-0 bg-black bg-opacity-90 flex flex-col items-center justify-center transition-transform duration-700 ease-in-out transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
